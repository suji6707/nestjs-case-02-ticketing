...
import { ConcertScheduleRequestDto, ConcertSeatRequestDto } from './dtos/request.dto';
import { ConcertSchduleResponseDto, ConcertSeatResponseDto } from './dtos/response.dto';
...
export class EventSearchController {
	...
	@ApiOkResponse({ type: ConcertSchduleResponseDto, description: '예약가능 날짜 조회 성공' })
	@UseGuards(AuthGuard)
	...
	async getSchedules(
		...
		return this.eventSearchService.getSchedules(userId, concertId, dto.queueToken);
	}

	// 예약가능 좌석 조회 - 대기열 토큰 검증
	@ApiBearerAuth()
	@ApiOperation({ summary: '예약가능 좌석 조회' })
	@ApiOkResponse({ type: ConcertSeatResponseDto, description: '예약가능 좌석 조회 성공' })
	@UseGuards(AuthGuard)
